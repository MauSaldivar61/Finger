...
rho_tpms = 0.33; % Percentage of solid material within the TPMS   use 0.33, 0.46, 0.58
...
design_num = 1; % How many variations of the design you wanna test?

for i = design_num
    
    % What's the name of the hard (H), soft (S), and Fluid (F, F1) files
%   filename_H = [foldername,'\Finger_hard_v',num2str(i),'.stl'];
    filename_H = [foldername,'\Finger_senseHard_v',num2str(i),'.stl'];
%   filename_S = [foldername,'\Finger_soft_v',num2str(i),'.stl'];
    filename_S = [foldername,'\Finger_senseBoundary_v',num2str(i),'.stl'];
%   filename_F = [foldername,'\Finger_TnF_v',num2str(i),'.stl']; % to generate Soft-fluid part with porous 
    filename_F = [foldername,'\Finger_senseTPMS_v',num2str(i),'.stl'];
%   filename_F1 = [foldername,'\Finger_F_v',num2str(i),'.stl'];  % to generate 100% fluid 
    filename_F1 = [foldername,'\Finger_senseLiquid_v',num2str(i),'.stl']; 
    ...
    plot2svg(['C:\Scratch\Finger\',name_struct,'_F_col.svg']);

    % MAKING A VIDEO
    makeVideosSIGMA( repelem( G_color_fluid, ceil(720/size(G_color_fluid,2)),ceil(720/size(G_color_fluid,2)))  ,['C:\Scratch\Finger\',name_struct,'_F_col'],12);
